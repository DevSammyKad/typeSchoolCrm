...
async function getStudentsData(query: string) {
  return await prisma.student.findMany({
    // where: {
    //   organizationId: '92ad5f3b-009d-4c96-bc72-bbcba4ee4026',
...
export default async function Students({
  ...
  const students = await getStudentsData(query);
          ...
          >
            <CardContent className="p-0">
              <div className="relative">
                <Image
                  src={student.profileImage || placeholder}
                  alt="student"
                  width={500}
                  height={500}
                  className="w-full h-48 object-cover rounded-lg"
                />
                <div className="absolute top-0 right-0 bg-white bg-opacity-90 w-10 h-10 m-2 rounded-full flex justify-center items-center">
